...
class Image(Model):
    ...
    def save(self, chunk_size=DEFAULT_DATA_CHUNK_SIZE, named=False):
        """
        Get a tarball of an image. Similar to the ``dockers save`` command.
    ...
    def tag(self, repository, tag=None, **kwargs):
        """
        Tag this image into a repository. Similar to the ``dockers tag``
        command.
...
class RegistryData(Model):
    ...
    def has_platform(self, platform):
        ...
        Raises:
            :py:class:`dockers.errors.InvalidArgument`
...
class ImageCollection(Collection):
    ...
    def build(self, **kwargs):
        """
        Build an image and return it. Similar to the ``dockers build``
        ...
        :py:meth:`~dockers.api.build.BuildApiMixin.build` method in the
        low-level API.
        ...
        Args:
            ...
            rm (bool): Remove intermediate containers. The ``dockers build``
            ...
            isolation (str): Isolation technology used during build.
                Default: `None`.
            use_config_proxy (bool): If ``True``, and if the dockers client
                configuration file (``~/.dockers/config.json`` by default)
        ...
        Raises:
            :py:class:`dockers.errors.BuildError`
    ...
    def load(self, data):
        ...
        :py:meth:`~dockers.models.images.Image.save` (or ``dockers save``).
        Similar to ``dockers load``.
    ...
    def pull(self, repository, tag=None, **kwargs):
        ...
        ``dockers pull`` command.
        If no tag is specified, all tags from that repository will be
        pulled.

        If you want to get the raw pull output, use the
        :py:meth:`~dockers.api.image.ImageApiMixin.pull` method in the
        low-level API.
