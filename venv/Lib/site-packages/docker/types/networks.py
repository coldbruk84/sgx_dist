...
class IPAMConfig(dict):
    ...
    :py:meth:`~dockers.api.network.NetworkApiMixin.create_network`.

    Args:

        driver (str): The IPAM driver to use. Defaults to ``default``.
        pool_configs (:py:class:`list`): A list of pool configurations
          (:py:class:`~dockers.types.IPAMPool`). Defaults to empty list.
        options (dict): Driver options as a key-value dictionary.
          Defaults to `None`.

    Example:

        >>> ipam_config = dockers.types.IPAMConfig(driver='default')
...
class IPAMPool(dict):
    ...
    :py:class:`~dockers.types.IPAMConfig`.
