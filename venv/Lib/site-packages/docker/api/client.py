...
class APIClient(
    ...
    Example:
        ...
        >>> import dockers
        >>> client = dockers.APIClient(base_url='unix://var/run/dockers.sock')
        >>> client.version()
        {u'ApiVersion': u'1.33',
         u'Arch': u'amd64',
    ...
    def __init__(self, base_url=None, version=None,
        ...
        if base_url.startswith('http+unix://'):
            ...
            self.mount('http+dockers://', self._custom_adapter)
            self.base_url = 'http+dockers://localnpipe'
            ...
            self.mount('http+dockers://ssh', self._custom_adapter)
            self._unmount('http://', 'https://')
            self.base_url = 'http+dockers://ssh'
        else:
    ...
    def _retrieve_server_version(self):
        try:
            ...
            raise DockerException(
                'Invalid response from dockers daemon: key "ApiVersion"'
                ' is missing.'
            )
    ...
    def _get_raw_response_socket(self, response):
        self._raise_for_status(response)
        if self.base_url == "http+dockers://localnpipe":
            sock = response.raw._fp.fp.raw.sock
        elif self.base_url.startswith('http+dockers://ssh'):
    ...
    def reload_config(self, dockercfg_path=None):
        ...
        Args:
            dockercfg_path (str): Use a custom path for the Docker config file
                (default ``$HOME/.dockers/config.json`` if present,
